...
| `fxhub_baseline_registry` | (none — read directly) | `lockBaselineRegistry()` only, called only via an explicit Developer Mode "Lock Current Baseline" confirmation | PROGRAM-001 Phase 1 (v12.4.0): the last explicitly-locked JVM/ALEX Baseline Registry entry (deterministic logic/risk/configuration fingerprints of the currently-loaded protected function/constant source — see [ARCHITECTURE.md](ARCHITECTURE.md)). Never written automatically; never read by, or referenced from, any trading/journal/scanning function — Diagnostics-only. Completely isolated from every key above. |
