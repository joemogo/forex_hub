...
const M=[
 ...
 ['K23 the pre-registered DRAWDOWN condition dropped from the verdict',
  'drawdownAcceptable:(b.maxDrawdown!=null&&b.maxDrawdown<=','drawdownAcceptable:(true||b.maxDrawdown<='],
 ['K24 a null drawdown treated as acceptable',
  'drawdownAcceptable:(b.maxDrawdown!=null&&','drawdownAcceptable:(b.maxDrawdown==null||'],
 ['K25 the drawdown limit loosened past the pre-registered 30%',
  'maxDrawdownLimit:0.30,','maxDrawdownLimit:0.95,'],
 ['K26 the Years box back to calendar days (asks 45% more history than it says)',
  'const CARRY_TRADING_DAYS_PER_YEAR=252;','const CARRY_TRADING_DAYS_PER_YEAR=365;'],
 ['K27 the bar-count cap removed',
  'return Math.min(Math.ceil(y*CARRY_TRADING_DAYS_PER_YEAR)+40,9000);',
  'return Math.ceil(y*CARRY_TRADING_DAYS_PER_YEAR)+40;'],
