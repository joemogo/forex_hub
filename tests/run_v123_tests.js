// Self-contained runner for the v12.3.0 TJR_SLR Phase 1 (Session and Zone Engine) fixture suite.
// Same extraction/stub pattern as every other tests/run_*_tests.js runner in this repo.
//
// Run from the project root:
//   cd "Forex Hub" && osascript -l JavaScript tests/run_v123_tests.js
// or simply:
...
const testCode=readFile('./tests/v123_tjr_phase1_session_zone_tests.js');
...
const wrapped = new Function('g',
  ...
  // -- TJR Phase 1 pure engine functions under test --
  'g.resolveTjrSessionBoundaries=resolveTjrSessionBoundaries;' +
  'g.getTjrSessionForTimestamp=getTjrSessionForTimestamp;' +
  'g.getPreviousCompletedTjrSession=getPreviousCompletedTjrSession;' +
  'g.isTjrSessionComplete=isTjrSessionComplete;' +
  'g.getCandlesForResolvedSession=getCandlesForResolvedSession;' +
  'g.findTjrSessionExtremes=findTjrSessionExtremes;' +
  'g.buildTjrHighZone=buildTjrHighZone;' +
  'g.buildTjrLowZone=buildTjrLowZone;' +
  ...
  'g.getDefaultTjrZonesToggle=function(){return chartOverlayToggles.tjrZones;};' +
  'return runV123Fixtures(g);'
);
const results = wrapped(g);
results.forEach(r=>console.log((r.pass?'PASS':'FAIL')+' -- '+r.name+(r.detail?' ('+r.detail+')':'')));
const failCount=results.filter(r=>!r.pass).length;
console.log('---');
console.log(failCount===0?'ALL v12.3.0 TJR PHASE 1 SESSION/ZONE FIXTURES PASSED':('FAILURES: '+failCount+'/'+results.length));
