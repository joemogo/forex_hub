// Self-contained runner for the G-2 scan-freshness (MOGO-024) fixture suite. Requires no separate
// extraction/preprocessing step -- reads index.html directly and extracts its <script> body
// itself, following the same pattern as run_v1213_tests.js.
//
...
const testCode=readFile('./tests/v1240_scan_freshness_tests.js');
...
const wrapped = new Function('g',
  ...
  'g.beginGen=function(){return jvmBeginEligibilityGeneration();};' +
  'g.invalidateAll=function(){return jvmInvalidateAllEligibility();};' +
  'g.markInProgress=function(p,gn){return jvmMarkEligibilityInProgress(p,gn);};' +
  'g.markFresh=function(p,gn){return jvmMarkEligibilityFresh(p,gn);};' +
  'g.markFailed=function(p,gn){return jvmMarkEligibilityFailed(p,gn);};' +
  'g.isCurrent=function(p){return jvmEligibilityIsCurrent(p);};' +
  'g.statusOf=function(p){return jvmEligibilityStatusOf(p);};' +
  'g.htfSnapshotOf=function(p){return htfSnapshotOf(p);};' +
  'g.htfAlignmentPasses=function(x){return htfAlignmentPasses(x);};' +
  'g.setScanData=function(v){scanData=v;};' +
  'g.getScanData=function(){return scanData;};' +
  'g.resetEligibility=function(){__jvmEligibility={};__jvmEligibilityGeneration=0;};' +
  'g.toggleAutoScan=function(on){document.getElementById("autoScanToggle").checked=!!on;toggleAutoScan();};' +
  'g.getAutoScan=function(){return autoScan;};' +
  'g.getAppSource=function(){return g.__src;};' +
  'return runG2Fixtures(g);'
);
g.__src=appCode;
...
console.log('---');
console.log(failCount===0?'ALL G-2 SCAN FRESHNESS FIXTURES PASSED':('FAILURES: '+failCount+'/'+results.length));
